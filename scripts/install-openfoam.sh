...
Install OpenFOAM-13 for Ubuntu, applying the patch required by hippo.
...
OPENFOAM_DIR="${OUT_DIR}/OpenFOAM-13"
OPENFOAM_REV="d5aebccff79d38838956e1353f198ca89139e666"
THIRDPARTY_DIR="${OUT_DIR}/ThirdParty-13"
THIRDPARTY_REV="3d868855b80c5d7de18f1f3793e8e5fe94bc168f"

# Fetch and patch OpenFOAM
mkdir -p "${OPENFOAM_DIR}"
if [ ! -d "${OPENFOAM_DIR}/.git" ]; then
    git clone https://github.com/OpenFOAM/OpenFOAM-13.git "${OPENFOAM_DIR}"
fi
...
if [ ! -d "${THIRDPARTY_DIR}/.git" ]; then
    git clone https://github.com/OpenFOAM/ThirdParty-13.git "${THIRDPARTY_DIR}"
fi
git -C "${THIRDPARTY_DIR}" reset --hard "${THIRDPARTY_REV}"
(
    cd "${THIRDPARTY_DIR}" \
    && export ZOLTAN_TYPE=none ./Allwmake
)
